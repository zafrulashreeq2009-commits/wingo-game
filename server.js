...
const JWT_SECRET = process.env.JWT_SECRET || 'wingo_jwt_secret_2024';
const ADMIN_PASS = process.env.ADMIN_PASSWORD || 'admin123';

const db = new Database('wingo.db');
db.pragma('journal_mode = WAL');
db.exec("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, player_id TEXT UNIQUE NOT NULL, phone TEXT UNIQUE NOT NULL, password TEXT NOT NULL, balance REAL DEFAULT 0.00, withdraw_count INTEGER DEFAULT 0, withdraw_date TEXT DEFAULT '')");
db.exec("CREATE TABLE IF NOT EXISTS game_history (id INTEGER PRIMARY KEY AUTOINCREMENT, player_id TEXT NOT NULL, period TEXT NOT NULL, bet_type TEXT NOT NULL, bet_amount REAL NOT NULL, result_number INTEGER NOT NULL, result_type TEXT NOT NULL, won INTEGER DEFAULT 0, win_amount REAL DEFAULT 0, balance_after REAL DEFAULT 0, created_at TEXT DEFAULT (datetime('now','localtime')))");
...
function getUser(pId) {
  return db.prepare('SELECT * FROM users WHERE player_id=?').get(pId);
}
...
app.post('/api/auth/login', (req, res) => {
  ...
  if(user) {
    ...
    const token = jwt.sign({player_id:user.player_id,phone:user.phone}, JWT_SECRET, {expiresIn:'30d'});
    return res.json({token,player_id:user.player_id,phone:user.phone,balance:user.balance||0,message:'Login berjaya!'});
  }
  ...
  const token = jwt.sign({player_id:pid,phone:p}, JWT_SECRET, {expiresIn:'30d'});
  res.status(201).json({token,player_id:pid,phone:p,balance:0,message:'Daftar berjaya! ID anda: '+pid});
});

app.get('/api/auth/me', auth, (req, res) => {
  const u = getUser(req.user.player_id);
  if(!u) return res.status(404).json({error:'Tak jumpa.'});
  res.json({player_id:u.player_id,phone:u.phone,balance:u.balance||0,withdraw_count:u.withdraw_count||0,withdraw_date:u.withdraw_date||''});
});
...
app.post('/api/game/bet', auth, (req, res) => {
  ...
  if(amt>uBal) return res.status(400).json({error:'Baki tak cukup.'});
  const rn = Math.floor(Math.random()*10);
  const rt = rn>=5?'BIG':'SMALL';
  const won = betType===rt;
  const wa = won?amt*1.96:0;
  const nb = won?(uBal-amt+wa):(uBal-amt);
...
app.get('/api/game/history', auth, (req, res) => {
  res.json(db.prepare('SELECT * FROM game_history WHERE player_id=? ORDER BY id DESC LIMIT 30').all(req.user.player_id));
});
...
app.post('/api/game/request-withdraw', auth, (req, res) => {
  ...
  res.json({success:true,telegramUrl:'https://t.me/moinsveit?text='+encodeURIComponent('WITHDRAW RM '+amt.toFixed(2)+' ID:'+req.user.player_id+' Baki:RM '+uBal.toFixed(2)),remainingWithdraws:2-(wc+1)});
});
